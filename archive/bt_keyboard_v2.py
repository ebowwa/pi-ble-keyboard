#!/usr/bin/env python3
"""
BLE HID Keyboard for Raspberry Pi 4B v2.
With pairing agent + Report Reference descriptor (iOS requirement).
...
class HIDInfoChar(Characteristic):
    def __init__(self, bus, index, service):
        ...
        self.value = [0x01, 0x01, 0x00, 0x03]  # CDH 1.1, Remote Wake + Connectable
...
class HIDReportChar(Characteristic):
    ...
    def StartNotify(self):
        ...
        global _report_char
        _report_char = self
        print('[+] Notifications enabled — keyboard ready')

    def send_key(self, hid_code, modifier=0):
        # Report ID 1 prepended (matches 0x85 0x01 in descriptor + Report Reference desc)
...
class ReportReferenceDesc(Descriptor):
    """iOS REQUIRES this. Report ID=1, Report Type=Input."""
...
# --- Pairing Agent (auto-accept everything) ---
...
class PairingAgent(dbus.service.Object):
    ...
    def DisplayPasskey(self, device, passkey, entered):
        print(f'[Agent] DisplayPasskey: {passkey:06d} ({entered} entered) for {device}')

    @dbus.service.method(AGENT_IFACE, in_signature='ou', out_signature='')
    def RequestConfirmation(self, device, passkey):
        print(f'[Agent] Auto-confirm passkey {passkey:06d} for {device}')

    @dbus.service.method(AGENT_IFACE, in_signature='o', out_signature='')
    def RequestAuthorization(self, device):
        print(f'[Agent] Auto-authorize: {device}')

    @dbus.service.method(AGENT_IFACE, in_signature='os', out_signature='')
    def AuthorizeService(self, device, service_uuid):
        print(f'[Agent] Authorize service {service_uuid} for {device}')
...
def type_text(text):
    global _report_char
    if not _report_char:
        print('[!] No client subscribed — connect & enable notifications first')
        return
    for ch in text:
        mod = 0x00
        code = None
...
def main():
    ...
    agent_mgr.RegisterAgent(PairingAgent.AGENT_PATH, 'KeyboardDisplay',
        reply_handler=lambda: print('[+] Pairing agent registered'),
        error_handler=lambda e: print(f'[!] Agent error: {e}'))
    agent_mgr.RequestDefaultAgent(PairingAgent.AGENT_PATH,
        reply_handler=lambda: print('[+] Default agent set'),
        error_handler=lambda e: print(f'[!] Default agent error: {e}'))
    ...
    print('[*] BLE HID Keyboard v2 running')
    print('[*] Pair as "Pi Keyboard"')
