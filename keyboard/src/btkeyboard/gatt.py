"""
BlueZ GATT server — D-Bus objects for the HID keyboard service.

Implements the org.bluez.GattService1 / GattCharacteristic1 / GattDescriptor1
interfaces that BlueZ discovers and serves to connected BLE clients (iOS).

Uses custom D-Bus paths under ``/pihid/`` to avoid conflicts with BlueZ's
...
# Global reference to the report characteristic (set when iOS subscribes)
_report_char = None


def get_report_char():
    """Return the active HIDReportChar instance, or None if not subscribed."""
    return _report_char
...
class Service(dbus.service.Object):
    """Base GATT service. Uses /pihid/ prefix to avoid BlueZ path conflicts."""
...
class Characteristic(dbus.service.Object):
    ...
    def send_notification(self, value):
        """Send a GATT notification with the given byte array."""
        self.value = list(value)
...
# ── HID-specific characteristics ──────────────────────────────────────────


class HIDInfoChar(Characteristic):
    """HID Information (0x2A4A) — read-only, returns version/country/flags."""
...
class HIDReportMapChar(Characteristic):
    """HID Report Map (0x2A4B) — read-only, returns the HID descriptor bytes."""
...
class HIDReportChar(Characteristic):
    """HID Report (0x2A4D) — notify-enabled, 8-byte Boot Protocol reports."""

    def __init__(self, bus, index, service):
        super().__init__(bus, index, HID_REPORT_UUID, ["read", "notify"], service)
        self.value = [0] * 8  # 8 bytes, NO Report ID prefix
    ...
    def send_key(self, hid_code, modifier=0):
        """Send a key press + release as two Boot Protocol notifications."""
        # Key press: [modifier, 0x00, keycode, 0, 0, 0, 0, 0]
        self.send_notification([modifier, 0x00, hid_code, 0x00, 0x00, 0x00, 0x00, 0x00])
        import time
        time.sleep(0.04)
        # Key release: all zeros
        self.send_notification([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
        time.sleep(0.04)


class HIDProtocolModeChar(Characteristic):
    """HID Protocol Mode (0x2A4E) — writable, Report Protocol Mode."""
...
class HIDCtrlPointChar(Characteristic):
    """HID Control Point (0x2A4C) — Suspend/Resume commands."""
...
class ReportReferenceDesc(Descriptor):
    """Report Reference (0x2908) — Report ID=0, Report Type=Input."""

    def __init__(self, bus, index, characteristic):
        super().__init__(bus, index, REPORT_REF_UUID, ["read"], characteristic)
        self.value = REPORT_REF_BOOT_KEYBOARD


class HIDService(Service):
    """Complete HID over GATT service with all required characteristics."""
