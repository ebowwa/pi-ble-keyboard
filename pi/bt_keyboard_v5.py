#!/usr/bin/env python3
"""
BLE HID Keyboard v5 - Pure Boot Protocol.
No Report ID. 8-byte reports. Simplest possible HID descriptor.
"""
import dbus
import dbus.service
...
# Standard HID Keyboard Report Descriptor - NO Report ID
# This is the exact descriptor used by real BLE keyboards
HID_REPORT_MAP = [
...
class HIDReportChar(Characteristic):
    ...
    def send_key(self, hid_code, modifier=0):
        # Pure 8-byte Boot Protocol report - NO Report ID prefix
...
class ReportReferenceDesc(Descriptor):
    # Report ID=0, Report Type=Input (Boot keyboard)
    def __init__(self, bus, index, characteristic):
        super().__init__(bus, index, REPORT_REF_UUID, ["read"], characteristic)
        self.value = [0x00, 0x01]  # Report ID=0, Input Report
...
def main():
    ...
    print("[*] BLE HID Keyboard v5 (Boot Protocol) running", flush=True)
